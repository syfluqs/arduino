...
#define CH5_1_MAX 999
#define CH5_1_MIN 968
#define CH5_2_MAX 1540
#define CH5_2_MIN 1440
#define CH5_3_MAX 2015
#define CH5_3_MIN 1985
//separate values for ch6 2-way switch
#define CH6_0_MAX 2005
#define CH6_0_MIN 1992
#define CH6_1_MAX 990
#define CH6_1_MIN 977
//LEFT RIGHT ZERO RANGE
#define LR_ZERO_MIN 1460
#define LR_ZERO_MAX 1480 
...
void loop()
{
  ...
  if (ch5>=CH5_1_MIN && ch5<CH5_1_MAX) {
    ...
    if (ch1<=LR_ZERO_MIN) {
      delta_right = 0;
      delta_left = map(ch1, CH1_MAX, CH1_MIN, 0, LINEAR_MODE_DELTA);
    }
    else if (ch1>=LR_ZERO_MAX) {
      delta_right = map(ch1, CH1_MIN, CH1_MAX, 0, LINEAR_MODE_DELTA);
      delta_left = 0;
    }
    else {
      delta_right = 0;
    ...
    if (forward>=255) {forward=255;}
    if (left_1>=255) {left_1=255;}
    if (right_1>=255) {right_1=255;}
  ...
  //ch5 switch set at 2
  //drift mode
  //bot moves with max velocity possible and exhibits sharp turns
    ...
    if (ch1<=LR_ZERO_MIN) {
      delta_right = 0;
      delta_left = map(ch1, CH1_MAX, CH1_MIN, -50, LINEAR_MODE_DELTA);
    }
    else if (ch1>=LR_ZERO_MAX) {
      delta_right = map(ch1, CH1_MIN, CH1_MAX, -50, LINEAR_MODE_DELTA);
      delta_left = 0;
    }
    else {
      delta_right = 0;
    ...
    if (left_1<0) {left_1=0;}
    if (right_1<0) {right_1=0;}
    if (left_2<0) {left_2=0;}
    if (right_2<0) {right_2=0;}
    if (forward>=255) {forward=255;}
    if (left_1>=255) {left_1=255;}
    if (right_1>=255) {right_1=255;}
    if (left_2>=255) {left_2=255;}
    if (right_2>=255) {right_2=255;}
  ...
  //not implemented yet
    ...
    if (ch1<=LR_ZERO_MIN) {
      delta_right = 0;
      delta_left = map(ch1, CH1_MAX, CH1_MIN, 0, M3_DELTA);
    }
    else if (ch1>=LR_ZERO_MAX) {
      delta_right = map(ch1, CH1_MIN, CH1_MAX, 0, M3_DELTA);
      delta_left = 0;
    }
    else {
      delta_right = 0;
  ...
  if ( (ch4 <= (CH4_MIN+200)) ) { reverse = true; }
  else {reverse = false; }
